...
function required(name: string) {
  ...
  if (!value) throw new Error(`Missing ${name}. Configure the Cloudflare R2 credentials.`);
  return value;
}
...
function config() {
  ...
  if (parsed.protocol !== "https:") throw new Error("R2_ENDPOINT must use HTTPS.");
  return {
    endpoint,
...
function signature(method: string, path: string, query: QueryEntry[], headers: Record<string, string>, payloadHash: string, amzDate: string, date: string, secret: string) {
  ...
  return {
    signedHeaders,
    scope,
    value: createHmac("sha256", signingKey(secret, date)).update(stringToSign).digest("hex"),
  };
}
...
async function signedRequest(method: string, key: string, query: QueryEntry[] = [], body = "", extraHeaders: Record<string, string> = {}) {
  ...
  const response = await fetch(`${settings.endpoint}${path}${query.length ? `?${canonicalQuery(query)}` : ""}`, {
    method,
    headers,
    body: body || undefined,
    cache: "no-store",
  ...
  if (!response.ok) {
    ...
    throw new Error(`Cloudflare R2 returned ${response.status}: ${message}`);
  }
  return response;
}
...
export async function createMultipartUpload(key: string, contentType: string) {
  ...
  if (!uploadId) throw new Error("Cloudflare R2 did not return a multipart upload ID.");
  return uploadId;
}

export async function listMultipartParts(key: string, uploadId: string) {
  const response = await signedRequest("GET", key, [["uploadId", uploadId]]);
  const payload = await response.text();
...
export function presignMultipartParts(key: string, uploadId: string, partNumbers: number[]) {
...
export async function completeMultipartUpload(key: string, uploadId: string, parts: CompletedPart[]) {
  ...
  const response = await signedRequest("POST", key, [["uploadId", uploadId]], body, { "content-type": "application/xml" });
...
export function createObjectUploadUrl(key: string) {
  return presignedUrl("PUT", key, [], 5 * 60);
}

export function createObjectDeleteUrl(key: string) {
  return presignedUrl("DELETE", key, [], 5 * 60);
}
