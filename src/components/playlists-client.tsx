...
import type { AccountPayload, LiveSessionRecord, PlaylistRecord } from "@/lib/domain";
...
export function PlaylistsClient() {
  ...
  async function playItem(item: PlaylistItem, continuePlaylist = false) {
    ...
    if (!item.moment.liveSessionId) {
      setSession(null);
      setSegmentIndex(-1);
      return setNotice("This moment was not captured from a live session.");
    }
    try {
      ...
      window.setTimeout(() => seekSelectedStart(item, next, firstSegment), 0);
    } catch (error) {
      setNotice(error instanceof Error ? error.message : "The clip could not be loaded.");
    }
  }

  function seekSelectedStart(item = selectedItem, sourceSession = session, index = segmentIndex) {
  ...
  function updatePlaybackTime(video: HTMLVideoElement) {
    if (!selectedItem || !segment) return;
  ...
  async function removeItem(item: PlaylistItem) {
    ...
    if (selectedItemId === item.id) { setSelectedItemId(null); setSession(null); setSegmentIndex(-1); }
  }
  ...
  return <div className="space-y-5">
    ...
    <Panel className="grid gap-4 p-4 lg:grid-cols-[minmax(15rem,1fr)_minmax(18rem,1fr)]">
      <label className="grid gap-2"><Label>Playlist</Label><Select value={playlistId} onChange={(event) => { setPlaylistId(event.target.value); setSelectedItemId(null); setSession(null); }}><option value="">Select a playlist</option>{playlists.map((item) => <option key={item.id} value={item.id}>{item.name} · {item.user.name}{item.visibility === "WORKSPACE" ? " · shared" : ""}</option>)}</Select></label>
    ...
    {playlist ? <div className="grid gap-4 xl:grid-cols-[22rem_minmax(0,1fr)]">
      ...
      <Panel className="overflow-hidden"><div className="flex items-center justify-between border-b border-white/10 p-3"><div>{selectedItem ? <><p className="font-semibold text-white">{selectedItem.moment.match.title}</p><p className="text-xs text-slate-500">{selectedItem.moment.momentType.name} · {formatTime(selectedItem.moment.startTimeSeconds)}–{formatTime(selectedItem.moment.endTimeSeconds)}</p></> : <p className="text-sm text-slate-500">Select a clip to review it.</p>}</div>{selectedItem?.moment.createdBy ? <Badge><Users size={11} className="mr-1" />{selectedItem.moment.createdBy.name}</Badge> : null}</div><div className="aspect-video bg-black">{segment?.playbackUrl ? <video key={`${selectedItemId}-${segment.id}`} ref={videoRef} src={segment.playbackUrl} controls autoPlay playsInline className="h-full w-full object-contain" onLoadedMetadata={() => seekSelectedStart()} onTimeUpdate={(event) => updatePlaybackTime(event.currentTarget)} onEnded={advanceSegment} /> : <div className="flex h-full items-center justify-center text-sm text-slate-600"><ListVideo className="mr-2" />No clip selected</div>}</div></Panel>
