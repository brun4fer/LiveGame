...
export function LiveWorkspace({ matchId }: { matchId: string }) {
  ...
  async function startLocalArchive(directory: LocalDirectoryHandle, partNumber: number, startTimeSeconds: number) {
    ...
    if (!mimeType) {
      ...
      throw new Error("This browser cannot record the complete match as MP4. Update Chrome or Edge and try again.");
    }
  ...
  return <div className="flex min-h-0 flex-col gap-2 xl:h-[calc(100dvh-6.5rem)] xl:overflow-hidden">
    <Panel className="flex shrink-0 flex-wrap items-stretch overflow-hidden xl:grid xl:grid-cols-[auto_minmax(0,1fr)_auto]">
      <div className="flex shrink-0 items-center gap-1 border-r border-white/10 px-1.5 max-md:w-full max-md:border-b max-md:border-r-0">
        <Link href="/" title="Matches" aria-label="Matches" className="inline-flex h-8 items-center gap-1 rounded-md px-1.5 text-[10px] font-semibold text-slate-400 hover:bg-white/[.06] hover:text-white"><ArrowLeft size={12} /><span className="hidden 2xl:inline">Matches</span></Link>
        <Badge className="max-w-36 truncate border-cyan-300/25 bg-cyan-300/10 text-[10px] text-cyan-100" title={match.title}>Live: {match.title}</Badge>
