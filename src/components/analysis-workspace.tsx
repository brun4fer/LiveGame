...
import { Archive, ArrowLeft, Check, ChevronsLeft, ChevronsRight, Clock3, Cloud, Download, FileVideo, Loader2, Pause, Pencil, Play, RotateCcw, Settings2, SlidersHorizontal, Trash2, Upload, X } from "lucide-react";
  ...
  async function exportAllMoments() {
    ...
    try {
      for (const [index, moment] of moments.entries()) {
        ...
        const paths = exportLayout === "grouped"
          ? folders.map((folder) => `${safeExportName(moment.momentType.name)}/${safeExportName(folder)}/${fileName}`)
          : [fileName];
