...
import { ArrowLeft, ChevronLeft, ChevronRight, ChevronsRight, Cloud, Crosshair, Download, FileVideo, Goal, Loader2, MapPin, Pause, Pencil, Play, RotateCcw, Save, Trash2, Upload, X } from "lucide-react";
...
import { VideoFullscreenButton } from "@/components/video-controls";
...
export function SubmomentWorkspace({ matchId }: { matchId: string }) {
  ...
  }, [liveReplayTarget?.command, selectedLiveSegment?.id, selectedLiveSegment?.playbackUrl, usingLiveRecording]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  return <div className="flex min-h-0 w-full flex-col gap-2 xl:h-[calc(100dvh-6.5rem)] xl:overflow-hidden">
    ...
    <div ref={workspaceRef} data-video-workspace className="submoment-layout relative grid min-h-0 flex-1 items-stretch gap-2 min-[900px]:grid-cols-[12rem_minmax(0,1fr)_20rem] min-[1400px]:grid-cols-[15rem_minmax(0,1fr)_22rem]">
      ...
      <div className="flex min-h-0 min-w-0 flex-col">
        <Panel className="flex min-h-0 flex-1 flex-col overflow-hidden"><div className="relative aspect-video min-h-72 shrink-0 bg-black xl:aspect-auto xl:min-h-0 xl:flex-1">{videoSourceUrl ? <video key={usingLiveRecording ? selectedLiveSegment?.id : "match-video"} ref={videoRef} src={videoSourceUrl} crossOrigin="anonymous" className="h-full w-full object-contain" playsInline onLoadedMetadata={(event) => { event.currentTarget.playbackRate = playbackRate; }} onTimeUpdate={handleTimeUpdate} onEnded={handleVideoEnded} onPlay={() => setPlaying(true)} onPause={() => setPlaying(false)} /> : <button type="button" onClick={() => fileInputRef.current?.click()} className="flex h-full min-h-72 w-full flex-col items-center justify-center p-6 text-center"><FileVideo size={38} className="text-leaf-400" /><h2 className="mt-3 text-sm font-bold text-white">{restoringVideo ? "Loading the match video…" : "Upload the match video"}</h2><p className="mt-1 max-w-md text-xs text-slate-500">The video will be stored privately in Cloudflare R2.</p>{match.video ? <p className="mt-3 rounded-md border border-white/10 bg-white/[.04] px-3 py-2 text-[10px] text-slate-400">Expected: {match.video.fileName} · {formatBytes(match.video.fileSize)}</p> : null}</button>}{uploading ? <div className="absolute inset-x-0 bottom-0 h-1 bg-white/10"><div className="h-full bg-cyan-300 transition-[width]" style={{ width: `${Math.round(uploadProgress * 100)}%` }} /></div> : null}</div><div className="shrink-0 border-t border-white/10 p-2"><input aria-label="Moment position" type="range" min={selectedMoment?.startTimeSeconds || 0} max={selectedMoment?.endTimeSeconds || 0.1} step={0.1} value={selectedMoment ? Math.max(selectedMoment.startTimeSeconds, Math.min(currentTime, selectedMoment.endTimeSeconds)) : 0} disabled={!hasVideoSource || !selectedMoment} onChange={(event) => seekWithinSelectedMoment(Number(event.target.value))} className="h-1.5 w-full cursor-pointer accent-cyan-300 disabled:cursor-not-allowed disabled:opacity-40" /><div className="mt-2 flex items-center gap-1 overflow-x-auto"><Button size="icon" className="h-8 w-8 shrink-0" title="Previous moment" aria-label="Previous moment" disabled={selectedIndex <= 0} onClick={() => selectMoment(moments[selectedIndex - 1])}><ChevronLeft size={15} /></Button><Button size="icon" className="h-8 w-8 shrink-0" title="Back 5 seconds (left arrow)" aria-label="Back 5 seconds" disabled={!hasVideoSource || !selectedMoment} onClick={() => seekWithinSelectedMoment(currentTime - 5)}><RotateCcw size={14} /></Button><Button size="icon" className="h-8 w-8 shrink-0" variant="primary" disabled={!hasVideoSource || !selectedMoment} title={playing ? "Pause" : "Play"} aria-label={playing ? "Pause" : "Play"} onClick={togglePlayback}>{playing ? <Pause size={15} /> : <Play size={15} />}</Button><Button size="icon" className="h-8 w-8 shrink-0" title="Forward 5 seconds (right arrow)" aria-label="Forward 5 seconds" disabled={!hasVideoSource || !selectedMoment} onClick={() => seekWithinSelectedMoment(currentTime + 5)}><ChevronsRight size={14} /></Button><Button size="icon" className="h-8 w-8 shrink-0" title="Next moment" aria-label="Next moment" disabled={selectedIndex < 0 || selectedIndex >= moments.length - 1} onClick={() => selectMoment(moments[selectedIndex + 1])}><ChevronRight size={15} /></Button><div className="flex h-8 shrink-0 overflow-hidden rounded-md border border-white/10">{[0.5, 1, 2, 4].map((rate) => <button key={rate} type="button" title={`${rate}x speed`} aria-label={`${rate}x speed`} onClick={() => setRate(rate)} className={`min-w-9 px-2 text-[10px] font-semibold transition ${playbackRate === rate ? "bg-leaf-400 text-ink-950" : "bg-white/[.04] text-slate-300 hover:bg-white/[.1]"}`}>{rate}x</button>)}</div><span className="ml-auto shrink-0 font-mono text-xs text-white">{formatTime(currentTime)} {selectedMoment ? <span className="text-slate-600">/ {formatTime(selectedMoment.endTimeSeconds)}</span> : null}</span><VideoFullscreenButton targetRef={workspaceRef} /></div></div></Panel>
      </div>
